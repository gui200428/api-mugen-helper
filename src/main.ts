...
async function bootstrap() {
  ...
  expressApp.use('/demos/serve', (req: Request, res: Response, next: NextFunction) => {
    ...
    express.static(slugDir, {
      ...
      maxAge: req.path.match(/\.(js|css|png|jpg|jpeg|gif|svg|woff|woff2|ttf|ico|webp)$/)
        ? '1y'
        : 0,
    })(req, res, (err) => {
  ...
  expressApp.use('/portfolio/serve', (req: Request, res: Response, next: NextFunction) => {
    const parts = req.path.split('/').filter(Boolean);
  ...
  app.use((req: Request, res: Response, next: NextFunction) => {
    if (req.path.startsWith('/demos/serve/') || req.path.startsWith('/portfolio/serve/')) {
      return next();
    }
    return helmet({
